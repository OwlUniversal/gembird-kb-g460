...
# This template has no special lighting zones active, ensuring uniform color changes.
...
def create_color_sequence(g, r, b):
    """
    Generates a 7-packet sequence for a given color using the clean template.
    Args:
    ...
    """
    new_sequence = []
    # The new color triplet to be written into the packets
    color_triplet = [g, r, b]

    for packet_template in TEMPLATE_SEQUENCE_CLEAN:
        # Convert the template to a mutable list of integers
        packet_list = list(packet_template)

        # The color data starts at Byte 8. We replace all subsequent bytes
        # with a repeating pattern of our new color.
        for i in range(8, 64, 3):
            if i + 2 < len(packet_list):
                packet_list[i:i+3] = color_triplet
        
        # Convert the list back to an immutable bytes object
...
def main():
    ...
    try:
        ...
        print("\n--- Gembird KB-G460 Uniform Color Control ---")
