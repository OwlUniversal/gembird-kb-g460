...
# Based on the "Perfect Green" capture, which is a known-good, clean state.
...
# Packet 2: Color Data Template from the "Perfect Green" capture.
TEMPLATE_SET_COLOR_PROPERTIES = bytes.fromhex(
    "043503062100000000060404ff0000ff000300000000000000000000"
    "ff0000"  # This is the placeholder for the Win Lock Indicator color
...
def create_true_static_color_sequence(r, g, b):
    """
    Constructs the full 3-packet sequence to guarantee a uniform static color.
    It writes the same RGB value to the Main Color and Win Lock Indicator locations.
    """
    # 1. Start with the "Prepare" packet.
    sequence = [COMMAND_PREPARE_STATIC]
    
    # 2. Create the "Color Data" packet from our clean template.
    packet_list = list(TEMPLATE_SET_COLOR_PROPERTIES)
    
    # *** THE FINAL, CORRECT LOGIC ***
    # Set the Main Color (Location 1: Bytes 14, 15, 16)
    packet_list[14] = r
    packet_list[15] = g
    packet_list[16] = b
    
    # Set the Win Lock Indicator Color (Location 2: Bytes 28, 29, 30)
    # By setting this to the same color, we guarantee a uniform appearance.
    packet_list[28] = r
    packet_list[29] = g
    packet_list[30] = b
...
def main():
    ...
    try:
        ...
        print("\n--- Gembird KB-G460 TRUE Color Control (Final Version) ---")
        print("This script provides uniform color, regardless of Win Lock state.")
        print("Enter RGB color values (e.g., '0 255 0' for green).")
        ...
        while True:
            try:
                ...
                print(f"Invalid input format: {ex}. Please try again.")
